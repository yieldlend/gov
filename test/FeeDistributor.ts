...
describe("FeeDistributor", () => {
  it("Should deploy properly", async function () {
    ...
    expect(await feeDistributor.locker()).eq(locker.target);
    expect(await stakingEmissions.feeDistributor()).eq(feeDistributor.target);
  });
  ...
  it("Should allocate rewards in the first epoch", async function () {
    ...
    // create a lock
    await token.approve(locker.target, e18 * 1000000n);
    await locker.createLock(e18 * 1000000n, 86400 * 365 * 4);

    expect(await vToken.balanceOf(emissions.target)).eq(10000000000n * e18);
    expect(await vToken.balanceOf(feeDistributor.target)).eq("0");
    ...
    expect(await vToken.balanceOf(feeDistributor.target)).eq(4807692n * e18);
  });
  ...
  it("Should distribute all the rewards to the user in the first epoch", async function () {
    ...
    await token.connect(other).approve(locker.target, e18 * 1000000n);
