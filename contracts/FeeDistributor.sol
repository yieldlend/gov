...
contract FeeDistributor is IFeeDistributor, ReentrancyGuard, Initializable {
    ...
    function initialize(
        ...
        token = IERC20(_token);
        locker = IYieldLocker(_votingEscrow);
    }
    ...
    function _findTimestampEpoch(
        ...
        for (uint256 index = 0; index < 128; index++) {
            ...
            IYieldLocker.Point memory pt = locker.pointHistory(mid);
    ...
    function _findTimestampUserEpoch(
        ...
        for (uint256 index = 0; index < 128; index++) {
            ...
            uint256 mid = (min + max + 2) / 2;
            IYieldLocker.Point memory pt = locker.userPointHistory(nftId, mid);
    ...
    function _checkpointTotalSupply(uint256 timestamp) internal {
        ...
        for (uint256 index = 0; index < 20; index++) {
            if (t > roundedTimestamp) break;
                ...
                IYieldLocker.Point memory pt = locker.pointHistory(epoch);
    ...
    function _claim(
        ...
        IYieldLocker.Point memory userPoint = locker.userPointHistory(
            nftId,
            userEpoch
        );
        ...
        IYieldLocker.Point memory oldUserPoint = IYieldLocker.Point(0, 0, 0, 0);
        ...
        for (uint256 index = 0; index < 50; index++) {
            ...
            if (weekCursor >= userPoint.ts && userEpoch <= maxUserEpoch) {
                ...
                if (userEpoch > maxUserEpoch)
                    userPoint = IYieldLocker.Point(0, 0, 0, 0);
